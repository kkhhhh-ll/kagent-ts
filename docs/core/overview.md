...
| 范式 | 执行策略 | 适用场景 | 复杂度 |
|------|----------|----------|--------|
| ReAct | Thought → Action → Observation 循环 | 简单问答、单步工具调用 | 低 |
| Plan-Solve | Plan → Execute → Answer 两阶段 | 多步骤复杂任务 | 中 |
| Fusion | 自动路由 + 计划 + 反思 | 不确定复杂度的通用场景 | 高 |
| Orchestrator | DAG 分解 + 并行调度 | 大规模多代理协作 | 最高 |
