...
| 条件 | 检测方式 | mode gate | 开销 |
|------|----------|-----------|------|
| **配置开启** | `precipitation: "post-hoc"` | 是 | 零 |
| **用户说"记住"** | `wantsRemember` 信号（正则） | **否**，无视 mode，直接强制触发 | 零 |
| **踩坑后成功** | `consecutiveFailures >= 2` 且最终成功 | 是 | 零（已有计数） |
...
| 问题 | 缓解措施 |
|------|----------|
| LLM 幻觉写入低质量技能 | 技能惰性加载，LLM 需显式激活才会生效；关键词匹配需精确命中 |
| 技能名路径遍历 | 正则检查（同 `FileSkillLoader`） |
| 技能数量膨胀 | `precipitated: true` 标记方便人工审查清理 |
| 沉淀失败阻塞主流程 | fire-and-forget (.catch) + 5 分钟 AbortController 硬超时 |
