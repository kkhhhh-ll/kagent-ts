...
PrecipitateAgent Fork 一个轻量的 ReAct Agent，拥有独立上下文和只读工具，不污染主 Agent 上下文。沉淀在后台执行，失败不阻塞——整个流程用 `.catch()` 包裹，沉淀失败不会影响用户拿到的答案。新技能在下一次 Agent 调用时自动生效。
...
| 沉淀失败阻塞主流程 | 后台 fire-and-forget + `.catch()` 双重防御，失败仅记录日志 |
