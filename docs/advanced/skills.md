...
| 特性 | 旧版：关键词匹配 | 新版：BM25 检索 |
|---|---|---|
| 检索方式 | 简单的关键词包含匹配 | BM25 统计检索 |
| 召回范围 | 仅 keywords 字段 | name + description + keywords + systemPrompt |
| 跨语言 | 不支持 | 通过在 keywords 中添加 CJK 词支持 |
| 分数过滤 | 无（命中即激活） | 双阈值过滤（比值 + 绝对最低） |
